...
// this immediately returns fail if one of the init items is corrupted
// does not panic if the a new item fails
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    std::cout << "Usage: ./HTtest use_custom"
    << std::endl;
  }
  int table_size= 10000000;
  int num_init_items = 4000000;
  int num_new_items= 4000000;
  int additional_reads_per_op = 9;
  int num_threads = 16;
  int use_custom= atoi(argv[1]);
